...
class UserDb extends UserRepository {
  ...
  /*
   * We pass fields instead of case classes because we may only need to update some
   ...
   * By passing specific fields we can create specific functions like updatePassword,
   * updateName, etc.
   */
